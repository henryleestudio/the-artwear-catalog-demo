...
document.querySelectorAll('.checkBoxes').forEach( box => {
  box.addEventListener("change", event => {
    console.log(event.target.parentNode.getElementsByClassName('delete')[0].innerText)
    console.log(event.target.checked)

    const task = event.target.parentNode.querySelectorAll('.task')[0].innerText
    
    console.log(task)
    console.log("we've reach this point")
    fetch('/upDateEntry', {
      method: 'put',
